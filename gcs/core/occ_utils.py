...
try:  # pythonocc-core (conda-forge)
    ...
    try:
        ...
        _map_shapes = getattr(topexp, "MapShapes", None)
    except Exception:
        _map_shapes = None
    if _map_shapes is None:
        try:
            from OCC.Core.TopExp import TopExp
            _map_shapes = getattr(TopExp, "MapShapes", None) or getattr(TopExp, "MapShapes_s", None)
        except Exception:
    ...
    try:
        ...
        _vol_props = _surf_props = _lin_props = None
    if _vol_props is None:
        try:
            from OCC.Core.BRepGProp import BRepGProp
            _vol_props = getattr(BRepGProp, "VolumeProperties", None) or getattr(BRepGProp, "VolumeProperties_s", None)
            _surf_props = getattr(BRepGProp, "SurfaceProperties", None) or getattr(BRepGProp, "SurfaceProperties_s", None)
            _lin_props = getattr(BRepGProp, "LinearProperties", None) or getattr(BRepGProp, "LinearProperties_s", None)
        except Exception:
    ...
    if _bnd_add is None:
        try:
            from OCC.Core.BRepBndLib import BRepBndLib
            _bnd_add = getattr(BRepBndLib, "Add", None) or getattr(BRepBndLib, "Add_s", None)
        except Exception:
            from OCC.Core.BRepBndLib import brepbndlib_Add as _bnd_add  # type: ignore

    try:
        from OCC.Core.BRep import BRep_Tool
        _vertex_pnt = getattr(BRep_Tool, "Pnt", None) or getattr(BRep_Tool, "Pnt_s", None)
    except Exception:
        _vertex_pnt = None
    if _vertex_pnt is None:
        from OCC.Core.BRep import BRep_Tool_Pnt as _vertex_pnt  # type: ignore

    try:
        from OCC.Core.BRepTools import breptools
        _brep_write = getattr(breptools, "Write", None)
        _brep_read = getattr(breptools, "Read", None)
        _uv_bounds = getattr(breptools, "UVBounds", None)
    except Exception:
        _brep_write = _brep_read = _uv_bounds = None
    if _brep_write is None:
        try:
            from OCC.Core.BRepTools import BRepTools
            _brep_write = getattr(BRepTools, "Write", None) or getattr(BRepTools, "Write_s", None)
            _brep_read = getattr(BRepTools, "Read", None) or getattr(BRepTools, "Read_s", None)
            _uv_bounds = getattr(BRepTools, "UVBounds", None) or getattr(BRepTools, "UVBounds_s", None)
        except Exception:
