...
from .fases import PhaseManager
...
class CADDocument:
    ...
    def __init__(self, name: str = "Documento"):
        ...
        #: fasi del modello (update model, proprietà per elemento, solutori)
        self.phases = PhaseManager()
        #: parametri globali aggiornabili (comando "update parameters")
    ...
    def set_parameter(self, nome: str, valore: float) -> None:
        """Imposta/aggiorna un parametro globale (stile ``update parameters``)."""
        self.parameters[str(nome)] = float(valore)
        self.notify("parameters_changed", {"nome": nome, "valore": float(valore)})

    def update_parameters(self, **kwargs) -> Dict[str, float]:
        """Aggiorna in blocco i parametri assegnati e ri-notifica il documento.

        Analogia con il comando OpenSees ``parameter update ...`` / refresh del
        modello nelle build parametriche: dopo l'aggiornamento le fasi e la
        GUI si riallineano ai nuovi valori.
        """
        for k, v in kwargs.items():
            self.parameters[k] = float(v)
        self.notify("parameters_changed", {"nomi": sorted(kwargs)})
