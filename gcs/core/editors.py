...
class GeometryEditor:
    ...
    # ------------------------------------------------------- trasformazioni
    def trasla(self, ids: Iterable[int], dx, dy, dz) -> int:
        """Traslazione delle entità indicate (con undo)."""
    ...
    def ruota(self, ids, px, py, pz, ax, ay, az, angolo_deg) -> int:
        entita = self._entities(ids)
        fatti = 0
        for e in entita:
            if e.shape is None:
    ...
    def scala(self, ids, fattore, centro=(0, 0, 0)) -> int:
        entita = self._entities(ids)
        fatti = 0
        for e in entita:
            if e.shape is None:
